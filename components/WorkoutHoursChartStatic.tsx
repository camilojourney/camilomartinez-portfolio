...
export default function WorkoutHoursChartStatic({ weeklyData, sportAverages }: Props) {
    if (weeklyData.length === 0) {
        return (
            <div className="flex items-center justify-center h-64">
                <div className="text-center">
                    <div className="text-white/60 text-xl mb-2">⏱️ No Workout Hours Data</div>
                    <div className="text-white/40">No workout hours data available for the last 3 months</div>
                </div>
            </div>
        );
    }
    ...
    const chartData = weeklyData.reduce((acc: any, item) => {
        ...
        if (existingWeek) {
            existingWeek[item.sport_name] = item.total_hours;
        } else {
            acc.push({
                week: item.week,
                [item.sport_name]: item.total_hours
            });
        }
        return acc;
    }, []);
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                        <BarChart data={formattedChartData} margin={{ top: 20, right: 30, left: 20, bottom: 80 }}>
                            ...
                            <YAxis
                                tick={{ fontSize: 11, fill: '#ffffff80' }}
                                stroke="rgba(255,255,255,0.3)"
                                label={{ value: 'Hours', angle: -90, position: 'insideLeft', fill: '#ffffff80' }}
                            />
                            ...
                            >
                                ...
                                <div className="text-center">
                                    <div className="text-xl font-bold text-white">
                                        {Number(sport.avg_hours_per_week).toFixed(1)}h
                                    </div>
                                    <div className="text-white/60 text-xs">avg/week</div>
                                    <div className="text-white/40 text-xs mt-1">
                                        {Number(sport.total_hours).toFixed(1)}h total
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="mt-6 bg-orange-500/10 backdrop-blur-sm rounded-lg p-4 border border-orange-400/20">
                        <div className="text-center">
                            ...
                            <div className="text-2xl font-bold text-orange-400">
                                {sportAverages.reduce((sum, sport) => sum + Number(sport.avg_hours_per_week), 0).toFixed(1)}h
                            </div>
                            <div className="text-orange-300/60 text-xs">hours/week</div>
                        </div>
                    </div>
                </div>
            </div>
