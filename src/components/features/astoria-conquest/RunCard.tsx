  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-2">
        <div>
          <h3 className="font-medium text-foreground">Run #{run.run_number}</h3>
      ...
      <div className="grid grid-cols-2 gap-2 text-sm">
        <div>
          ...
          <span className="text-foreground">{(run.distance_meters / 1000).toFixed(2)} km</span>
        </div>
        <div>
        ...
        <div>
          ...
          <span className="text-foreground">{(run.average_speed_mps * 3.6).toFixed(1)} km/h</span>
        </div>
        <div>
          <span className="text-gray-400">Suffer Score: </span>
          <span className="text-foreground">{run.suffer_score ?? 'N/A'}</span>
        </div>
        <div>
          <span className="text-gray-400">WHOOP Strain: </span>
          <span className="text-foreground">{run.whoop_strain ? run.whoop_strain.toFixed(1) : 'N/A'}</span>
        </div>
        <div>
          <span className="text-gray-400">Avg HR: </span>
          <span className="text-foreground">{run.avg_heart_rate ? `${run.avg_heart_rate} bpm` : 'N/A'}</span>
        </div>
      </div>
