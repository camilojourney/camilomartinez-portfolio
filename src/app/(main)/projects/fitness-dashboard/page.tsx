      ...
      <div className="space-y-12">
          ...
          <Card className="border-white/10 bg-white/[0.05] p-8 md:p-10 space-y-6">
            ...
            <ul className="list-disc list-inside space-y-4 text-muted-foreground text-base md:text-lg">
              <li><span className="text-foreground font-medium">WHOOP API ingestion</span> via authenticated collectors pushing to TimescaleDB with automatic backfills.</li>
              <li><span className="text-foreground font-medium">Strava webhooks</span> keep activities synchronized without polling and trigger enrichment jobs.</li>
              <li><span className="text-foreground font-medium">PostgreSQL + TimescaleDB</span> for hypertables, retention policies, and downsampled materialized views.</li>
              <li><span className="text-foreground font-medium">Next.js 14</span> Server Components streaming analytics to interactive charts with ISR refresh every 6 hours.</li>
            </ul>
          </Card>

          <Card className="border-white/10 bg-white/[0.05] p-8 md:p-10 space-y-6">
            <h2 className="text-2xl md:text-3xl font-semibold text-foreground">Key Challenges Solved</h2>
            <p className="text-muted-foreground leading-relaxed text-base md:text-lg">
          ...
          <Card className="border-white/10 bg-gradient-to-br from-emerald-500/15 to-cyan-500/10 border-emerald-400/30 p-8 md:p-10 text-center space-y-4">
            <h3 className="text-2xl font-semibold text-foreground">See it in action</h3>
            <p className="text-muted-foreground">Explore the live dashboard, updated automatically as new WHOOP and Strava data arrives.</p>
            <Link
              href="/apps/fitness-dashboard"
              className="inline-flex items-center gap-2 rounded-xl border border-white/20 bg-white/10 px-6 py-3 text-foreground font-medium transition-all duration-300 hover:scale-105 hover:border-emerald-300/60 hover:text-emerald-100"
            >
            View Live Dashboard →
          </Link>
        </Card>
