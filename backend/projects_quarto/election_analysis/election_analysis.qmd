...
This project explores the United States Electoral College system and investigates whether alternative allocation methods for electoral votes would result in outcomes more representative of the popular vote. Using historical election data, we simulate various scenarios—including proportional allocation—to assess their impact on presidential election results (1976–2020).

## Introduction

In the United States, the President isn’t chosen directly by popular vote. Instead, the winner is determined through the [Electoral College](https://en.wikipedia.org/wiki/United_States_Electoral_College). With the 2024 Presidential election drawing near, we ask: **Would a different allocation method for electoral votes yield results that better reflect the national popular vote?**
...
We see clearly that the USA falls in the middle of the list, while the states that lost the most seats appear at the bottom.
...
Now we plot the shapefile data to visualize the boundaries of New York City boroughs.
...
First, we join the Electoral College votes to the presidential data previously assigned.
```{r}
...
This first plot shows the ECVs over time for the State-Wide Winner-Take-All allocation method. We can see how the ECVs are distributed between the Democratic and Republican parties in each election year, fluctuating over time.
...
This confirms that the total ECVs for each year are close to 538, verifying that the allocation method is functioning correctly.
...
```
Now we compare all these methods to observe the differences between them.
