...
from app.models.whoop import WHOOPRecovery, WHOOPSleep
...
class QueryProcessor:
    ...
    async def process_query(
        ...
        try:
            ...
            raise QueryProcessingError(f"AI processing failed: {e}") from e
...
Recent activities:"""
    ...
    async def get_query_history(
        ...
        try:
            ...
            raise QueryProcessingError(f"Failed to get query history: {e}") from e
        ...
        try:
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if attempt == max_attempts - 1:
                        raise QueryProcessingError(f"Failed to parse AI response after {max_attempts} attempts") from e

                except Exception as e:
                    logger.error(f"SQL generation attempt {attempt + 1} failed: {e}")
                    if attempt == max_attempts - 1:
                        raise QueryProcessingError(f"SQL generation failed: {e}") from e

            raise QueryProcessingError("Unexpected error in SQL generation loop")

        except Exception as e:
            logger.error(f"Error in SQL generation: {e}")
            raise QueryProcessingError(f"Failed to generate SQL query: {e}") from e
    ...
    async def execute_safe_query(
        ...
        try:
            ...
            raise QueryProcessingError(f"Query execution failed: {e}") from e
