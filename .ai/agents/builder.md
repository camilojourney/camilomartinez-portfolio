...
Load `contexts/camilomartinez-portfolio.md` for full product context.

```yaml
Framework: Next.js + FastAPI
Language: TypeScript + Python
Database: PostgreSQL/SQLite
Frontend: Next.js; Backend: FastAPI; Tooling: pnpm + uv
Testing: pytest + custom Node integration scripts
...
**Symptoms:** `pnpm test:db-schema` exits with errors
...
3. Clear local cache and rebuild: `pnpm + uv clean-install`
...
5. Lock file needs regeneration: `pnpm + uv install`
...
|--------|--------|-------------|
| Test coverage | ≥80% on new code | `pnpm test:db-schema:coverage` |
| Lint errors | 0 new errors | `pnpm exec next lint` |
| Type errors | 0 | `pnpm build` |
