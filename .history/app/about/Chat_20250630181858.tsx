            ...
            <div className="h-[28rem] overflow-y-auto p-6 space-y-4 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
                ...
                <div className="flex items-start gap-3 justify-start">
                    <Image 
                        ...
                        width={40} 
                        height={40} 
                        className="rounded-full border-2 border-cyan-400/30 shadow-lg flex-shrink-0"
                    />
                ...
                {messages.map((msg) => (
                    <div key={msg.id} className={`flex items-start gap-3 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                        ...
                        {msg.sender === 'user' && (
                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-cyan-400/20 to-blue-500/20 border-2 border-cyan-400/30 flex items-center justify-center shadow-lg flex-shrink-0">
            ...
            <div className="liquid-glass-input-container backdrop-blur-xl bg-white/[0.03] border-t border-white/[0.08] p-6">
                ...
                <div className="mt-6 pt-4 border-t border-white/[0.08]">
                    ...
                    <div className="flex flex-wrap gap-2">
                        <button 
                            onClick={() => setInput('What are his technical skills?')} 
                        ...
                        <button 
                            onClick={() => setInput('Tell me about Camilo story')} 
                            className="liquid-glass-suggestion-btn backdrop-blur-lg bg-white/[0.04] border border-white/[0.08] text-white/80 px-4 py-2 rounded-xl text-sm font-medium hover:bg-white/[0.08] hover:border-white/[0.12] hover:text-white transition-all duration-300 transform hover:scale-[1.02] shadow-lg"
                        >
                            📖 His Story?
                        </button>
                        <button 
                            onClick={() => setInput('What did he do before tech?')} 
                            className="liquid-glass-suggestion-btn backdrop-blur-lg bg-white/[0.04] border border-white/[0.08] text-white/80 px-4 py-2 rounded-xl text-sm font-medium hover:bg-white/[0.08] hover:border-white/[0.12] hover:text-white transition-all duration-300 transform hover:scale-[1.02] shadow-lg"
                        >
                            🔄 Past Experience?
                        </button>
