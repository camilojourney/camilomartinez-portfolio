...
class SimpleCorrelationETL {
  ...
  async processCorrelations(days = 30) {
    console.log(`🔄 Starting cross-platform activity correlation (${days} days)...`);
...
async function main() {
  ...
  try {
    if (process.argv[2] === 'create-table') {
      ...
      await etl.processCorrelations(days);
      ...
      console.log('  node run-correlation-etl.mjs process [days]   # Process new correlations (default: 30 days)');
      ...
      console.log('  node run-correlation-etl.mjs process          # Process last 30 days');
      console.log('  node run-correlation-etl.mjs process 90       # Process last 90 days (initial backfill)');
      console.log('  node run-correlation-etl.mjs process 7        # Process last 7 days (weekly sync)');
